...
function getThisField(field) {
    const attr2ignore = ['style', 'placeholder', 'required', 'maxlength', 'aria-required', 'autocomplete', 'spellcheck'];
...
function showFieldsMetadata() {
    ...
    if (inputs.length < 1) {
        console.error(`${manifest.name ?? ''}: Either field or form is invalid`, field);
        return;
    }
